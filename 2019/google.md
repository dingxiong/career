## google
- coding
    - [x] LC medium 56 62, 63 139 146 287 294 399 676 684 750 807 853 855
    ...
    - [x] 建水井，一个村庄里有很多房子，有的房子旁边能建水井，有的房子只能通过与其他房子建立管道获得供水。给出了在不同房子旁边建水井的花费，已及两个房子间建管道的花费，要求结果给出一份竞价，
        使得总花费尽可能少
        => optimal scenario is 多个联通分量，每个联通分量内只有一个水井，联通分量和最小。 这个可以转化为 加了一个source node，source node 连接到每一个能够建井的住户，
            每条边的开销就等于建井的开销，这时加上原来的建水管的开销，构成了一个新的图，在这个图上做最小生成树
    - [x] 第一轮 写一个区间查询的类，给 N 个区间和一个目标值，找出所有包含目标值的区间。查询函数会被频繁使用，要求至少做到O(logN), 且所有区间的端点和目标值都为实数，即会有无穷多个。
        follow up，如果区间会被增删，怎么做。
        => bucket preprocessing + binary search. Follow up: TreeMap
    - [x] 个地图上，有若干个地铁站，给一个数组，表示地铁站的坐标。地图范围为0<=x<=N-1, 0<=y<=M-1,假设空间是离散的，即 x, y 是整数，找出一个离所有地铁站的最近距离最远的空地，返回它的坐标。
        follow up 要求时间复杂度O(MxN)， follow up 把程序改成多线程。
        => collection BFS + visited matrix
    - [x] 有N个数，他们的取值范围为[0,N-2]。证明，N个数中至少有两个相同的数。找出两个相同的数，返回其index。
        分别用 1.时间O(N)空间O(N); 2.时间O(N^2),空间O(1); 3.时间O(NlogN)空间O(1), 4.时间O(N),空间O(1)来实现。
        => lc 287 
        (1) 时间O(N)空间O(N) => HashMap. (2) 时间O(N^2),空间O(1) => 2 for loops (3) 时间O(NlogN)空间O(1) => binary search (4)时间O(N),空间O(1) => find cycle
