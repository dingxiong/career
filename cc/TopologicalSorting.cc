...
public:
  ...
  vector<string> Kahn(vector<string> vertices,
  ...
  vector<string> DFS(vector<string> vertices,
                     vector<pair<string, string>> edges) {
    unordered_map<string, unordered_set<string>> dep;
    for (auto &p : edges)
      dep[p.first].insert(p.second);

    unordered_map<string, bool> permanent_mark, mark;
    for (string s : vertices) {
      permanent_mark[s] = false;
      mark[s] = false;
    }
    ...
    for (auto &s : vertices) {
      if (!permanent_mark[s]) {
        if (!visit(dep, path, permanent_mark, mark, s))
          return {};
      }
    }
   ...
   */
  bool visit(unordered_map<string, unordered_set<string>> &dep,
             vector<string> &path, unordered_map<string, bool> &permanent_mark,
             unordered_map<string, bool> &mark, string v) {
    if (mark[v])
      return false;
    mark[v] = true;
    for (auto nei : dep[v]) {
      if (!permanent_mark[nei]) {
        if (!visit(dep, path, permanent_mark, mark, nei))
          return false;
      }
    }
    mark[v] = false;
...
int main() {
  ...
  vector<pair<string, string>> edges = {
      {"5", "11"}, {"7", "11"},  {"7", "8"},  {"3", "8"}, {"11", "2"},
      {"11", "9"}, {"11", "10"}, {"3", "10"}, {"8", "9"}, {"7", "8"},
      {"7", "8"},  {"7", "8"}}; // add some dupilcates
